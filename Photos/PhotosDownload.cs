...
namespace Photos
{
    public static class PhotosDownload
    {
        ...
        public static async Task<IActionResult> Run(
            ...
            ILogger logger)
        {
            logger?.LogInformation($"Downloading photo id: {id} ...");

            byte[] data;

            if (req.Query["size"] == "sm")
            {
                logger?.LogInformation("Obtendo imagem pequena");
                data = await GetBytesFromStreamAsync(imageSmall);
            }
            else if (req.Query["size"] == "md")
            {
                logger?.LogInformation("Obtendo imagem média");
                data = await GetBytesFromStreamAsync(imageMedium);
            }
            else
            {
                logger?.LogInformation("Obtendo imagem original");
